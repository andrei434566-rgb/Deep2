...
def train_description_model(
    ...
    *,
    ...
    required_text_length = max(len(_clean_text(row["target_text"])) + 2 for row in train_rows + val_rows)
    if max_text_length is None:
        max_text_length = required_text_length
    if max_text_length < required_text_length:
        raise ValueError(
            f"Краткое описание будет обрезано: требуется max_text_length не менее {required_text_length}."
        )
    if image_size < 32 or hidden_size < 1 or batch_size < 1:
        raise ValueError("image_size должен быть не меньше 32, hidden_size и batch_size — положительными.")
    ...
    class CaptionDataset(Dataset):
        ...
        def __getitem__(self, index):
            ...
            return image_tensor, torch.tensor(facies_id, dtype=torch.long), torch.tensor(encoded, dtype=torch.long)

    def collate_batch(items):
        # Pad to this batch's longest text, not the whole well's longest text.
        # One long geological description must not inflate every training batch.
        images, facies_ids, sequences = zip(*items)
        return (
            torch.stack(images), torch.stack(facies_ids),
    ...
    train_loader = DataLoader(CaptionDataset(train_rows), batch_size=max(1, batch_size), shuffle=True, num_workers=0, collate_fn=collate_batch)
    val_loader = DataLoader(CaptionDataset(val_rows), batch_size=max(1, batch_size), shuffle=False, num_workers=0, collate_fn=collate_batch)
    best_loss = math.inf
    stale_epochs = 0
    best_epoch = 0
...
class DescriptionGenerator:
    """One loaded text network per analysis, including a standalone unified best.pt."""
    ...
    def __init__(self, model_path: Path | dict):
        ...
        if isinstance(checkpoint, dict) and "core_description_checkpoint" in checkpoint:
            checkpoint = checkpoint["core_description_checkpoint"]
        self.model, self.checkpoint = _load_description_network(checkpoint)

    def generate(self, image: Path | np.ndarray, facies: str = "") -> str:
        import torch
        ...
        prepared = _prepare_crop(image, int(checkpoint["image_size"]))
        image_tensor = torch.from_numpy(prepared.transpose(2, 0, 1)).float().unsqueeze(0)
        model = self.model
...
def _read_crop(path: Path, image_size: int) -> np.ndarray:
    return _prepare_crop(path, image_size)
...
def _prepare_crop(source: Path | np.ndarray, image_size: int) -> np.ndarray:
    ...
    if image is None or image.size == 0 or image.ndim != 3 or image.shape[2] != 3:
        raise ValueError("Не удалось открыть вырезку керна.")
