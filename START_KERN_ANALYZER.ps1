...
function Show-LaunchError([string]$message) {
    Write-Host ''
    Write-Host 'Kern Analyzer did not start:' -ForegroundColor Red
...
try {
    ...
    if (-not (Test-Path -LiteralPath $python)) {
        throw 'Python 3.11 or newer was not found. Install Python, then start Kern Analyzer again.'
    }

    Write-Host 'Checking Kern Analyzer modules...'
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host 'Installing Kern Analyzer components...'
    ...
    Write-Host 'Check passed. Kern Analyzer is opening.'
}
catch {
